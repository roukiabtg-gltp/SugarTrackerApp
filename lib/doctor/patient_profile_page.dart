...
Widget _statusChip(String s) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder(
        ...
        builder: (context, AsyncSnapshot<DatabaseEvent> uSnap) {
          // ✅ نستنى البيانات - لا نعرض شيء حتى تصل
          ...
          String mrn     = "MRN-${widget.patientId.substring(0, 5).toUpperCase()}";
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Container(
                    ...
                    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      // ✅ إذا كان الرابط موجود وغير فارغ يعرض الصورة من الإنترنت
                         backgroundImage: (profileImageUrl != null && profileImageUrl.isNotEmpty)
                            ? NetworkImage(profileImageUrl)
                               : null,
                               // ✅ إذا لم توجد صورة يعرض الحروف الأولى (initials)
        ...
        ClipRRect(
          ...
          columnWidths: const {
            ...
            children: [
              ...
              // Rows
              ..._sorted(meas).map((entry) {
                var v = entry.value;
                // ✅ يدعم category (جهاز) و type (طبيب)
                String type     = v['category']?.toString() ?? v['type']?.toString() ?? "--";
                // ✅ يدعم timestamp (رقمي) و date (ISO)
                dynamic timeRaw = v['timestamp'] ?? v['date'];
                String status   = _getStatus(type, v['value']);
                // ✅ doctor_added = true => Doctor, غير ذلك => Device
  ...
  showDialog(
    ...
    builder: (ctx) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            final data = {"value": _ctrl.text, "type": _type, "doctor_added": true};
            if (measId == null) {
  ...
  Future<void> _save(BuildContext ctx) async {
    ...
    String unit = {
      "Glucose": "mg/dL", "Blood Pressure": "mmHg",
      "Heart Rate": "bpm", "Weight": "kg", "Temperature": "°C"
    }[_type] ?? "mg/dL";
    await _db.child('measurements').child(widget.patientId).push().set({
      "type": _type,
